...
### remove FACTS planted plots, if they had trees in them
plots.exclude <- read.csv("data_intermediate/plots_exclude_FACTS.csv",header=TRUE,stringsAsFactors=FALSE)
plots.exclude = plots.exclude %>%
  dplyr::filter(planted == 1)

# determine which plots had zero trees in them
d.sp.all = d.sp %>%
  dplyr::filter(species == "CONIF.ALLSP",
                regen.count.all == 0)

# remove from the FACTS managed plots list those plots that had zero trees
plots.exclude = plots.exclude %>%
  dplyr::filter(!(Regen_Plot %in% d.sp.all$Regen_Plot))

plot.ids.exclude <- plots.exclude$Regen_Plot
d.plot <- d.plot[!(d.plot$Regen_Plot %in% plot.ids.exclude),]
...
# Remove tree data not from focal plots
focal_plots = unique(d.plot$Regen_Plot)
d.sp = d.sp[d.sp$Regen_Plot %in% focal_plots,]

## write
write.csv(d.plot,"data_intermediate/plot_level_pub.csv",row.names=FALSE)
write.csv(d.sp,"data_intermediate/speciesXplot_level_pub.csv",row.names=FALSE)

## read
d.plot <- read.csv("data_intermediate/plot_level_pub.csv",header=T,stringsAsFactors=FALSE)
d.sp <- read.csv("data_intermediate/speciesXplot_level_pub.csv",header=T,stringsAsFactors=FALSE)
...
write.csv(d.fire,"../tables/T1_V2F.csv")
...
write.csv(d.cat,"../tables/topotclim_cat_table_regenCounts_V2F.csv")
...
Cairo(file=paste0("../Figures/FigS1_ref_vs_highsev_V2F",Sys.Date(),".png"),width=2800,height=2000,ppi=200,res=200,dpi=200)
p
dev.off()
...
Cairo(file=paste0("../Figures/Fig1_climateSpace_2pptanom_",Sys.Date(),"_V2F.tif"),width=3100*3,height=1500*3,ppi=200*3,res=200*3,dpi=200*3) 
...
Cairo(file=paste0("../Figures/Fig1_climateSpace_2AETanom_",Sys.Date(),"_V2F.png"),width=3100,height=1500,ppi=200,res=200,dpi=200) 
grid.arrange(p1,blank,p2,ncol=3,widths=c(0.49,0.02,0.49))
dev.off()
...
sink("../run_output_V2F.txt") # this file will store model fits etc
for(sp in resp.opts) {
...
save.image("../model_workspace_V2F.Rwkspc")
#load("../model_workspace.Rwkspc")
load("../model_workspace_V2F.Rwkspc")
...
tiff(file=paste0("../Figures/FigX_full_prediction_plots_",Sys.Date(),"_FACTS.tiff"),width=3000,height=1200,res=200) 
p
dev.off()
...
tiff(file=paste0("../Figures/FigX_prediction_plots_",Sys.Date(),"_V2F.tiff"),width=6000,height=1600*3,res=600) 
...
tiff(file=paste0("../Figures/FigX_fitted_vs_observed_PPT_",Sys.Date(),"_V2F.tiff"),width=2100,height=1600,res=200) 
p
dev.off()
...
tiff(file=paste0("../Figures/FigX_fitted_vs_observed_AET_",Sys.Date(),"_V2F.tiff"),width=2100,height=1600,res=200) 
p
dev.off()
...
tiff(file=paste0("../Figures/FigX_CCA_",Sys.Date(),"_V2F.tiff"),width=1500,height=1500,res=200) 
p
dev.off()
