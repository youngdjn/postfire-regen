...
from <- c("ABCO","PIPJ","HDWD.ALLSP")
to <- c("White fir","Yellow pine","Broadleaf trees")
d$species.common <- mapvalues(d$species,from,to)

d.plot <- d[d$survey == "Revisit",]
d.plot <- d.plot[d.plot$species.common %in% to,]
d.plot$prop <- 100*d.plot$prop

d.plot$orig.presab <- ifelse(d.plot$orig.presab,"Yes","No")

ggplot(d.plot,aes(x=orig.presab,y=prop)) +
  geom_bar(stat="identity",position="dodge",width=0.5) +
  facet_grid(~species.common) +
  theme_bw(14) +
  geom_text(aes(label=labeltext,y=-2)) +
  labs(x="Regeneration present at initial survey",y="Percentage of plots with regeneration at revisit")
