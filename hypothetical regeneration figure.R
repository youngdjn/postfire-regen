...
d.dry <- data.frame(s,prob=dry,norm="Dry")
d.wet <- data.frame(s,prob=wet,norm="Wet")
...
p <-ggplot(d,aes(x=s,y=prob,color=norm)) +
  ...
  theme_bw(16)+
  labs(x="Post-fire precipitation anomaly",y="Seedling recruitment",color="Normal climate") +
  geom_vline(xintercept=0,linetype="dashed") +
  theme(axis.text.x = element_text(size=14),axis.text.y = element_text(size=14)) +
  scale_x_continuous(breaks=c(-2.2,0,2.2), labels= c("unusually dry","0","unusually wet")) +
